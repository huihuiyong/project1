...
{
    HAL_SPI_Transmit(&hspi4, &cmd, 1, 0xfff);
}

uint8_t own_HAL_SPI_Receive()
{
    uint8_t temp;
    HAL_SPI_Receive(&hspi4, &temp, 1, 0xfff);
    return temp;
}
/*******************************************************************************nandflash API*******************************************************************************/
void (*Flash_Delay)(rt_int32_t time) = &rt_thread_mdelay;    //
void (*Flash_SendByte)(uint8_t) = &own_HAL_SPI_Transmit;   //
uint8_t (*Flash_ReciByte)() = &own_HAL_SPI_Receive;   //
